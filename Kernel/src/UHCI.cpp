...
static void onSharedInterrupt(void* me) {
    UhciUsbController* self = (UhciUsbController*)me;
    self->onInterrupt();
}
...
UhciUsbController::UhciUsbController(uint8_t bus, uint8_t device, uint8_t function, const PCICommonHeader& header) {
    if((header.headerType & (~0x80)) != 0x00)
        return;
    kout << "UHCI controller not implemented\n";
    return;
    SharedInterrupt* interrupt;
    if(!SharedInterrupt::configInterrupt(bus, device, function, header, &interrupt)) {
        kout << "Could not config interrupt\n";
        return;
    }
    kout << "Config interrupt\n";

    interrupt->setData(this);
...
void UhciUsbController::onInterrupt() {
